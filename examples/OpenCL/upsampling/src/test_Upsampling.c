...
/* Example of Upsampling-by-2 Filter */
...
int main(int argc, char * argv[]) {
  ...
  printf("***********************************************************\n");
  printf("*              Example of Upsampling filter               *\n");
  printf("***********************************************************\n");
  printf("\n");
  ...
  cl_program program = createUpsamplingProgramFromBinary(context, device);
#else
  /* create a CL program using the kernel source */
  cl_program program = createUpsamplingProgramFromSource(context, device,
#ifdef DATA_INT
								    "-DDATA_INT"
#else
								    NULL
  ...
  cl_int *output = oclCreateMapBuffer(context,commandQueue,CL_MEM_READ_WRITE,CL_MAP_READ|CL_MAP_WRITE,sizeof(cl_int),4*IMAGE_X*IMAGE_Y,&outputBuffer);
#else
  ...
  cl_float *output = oclCreateMapBuffer(context,commandQueue,CL_MEM_READ_WRITE,CL_MAP_READ|CL_MAP_WRITE,sizeof(cl_float),4*IMAGE_X*IMAGE_Y,&outputBuffer);
#endif
  DATA_TYPE *check_tmp    = malloc(sizeof(DATA_TYPE)*IMAGE_Y*2*IMAGE_X);
  DATA_TYPE *check_output = malloc(sizeof(DATA_TYPE)*4*IMAGE_Y*IMAGE_X);
  
  /* For using arrays instead of pointers */
  DATA_TYPE (*in)[IMAGE_Y][IMAGE_X]  	      =(DATA_TYPE (*)[IMAGE_Y][IMAGE_X])input;
  DATA_TYPE (*out)[2*IMAGE_Y][2*IMAGE_X]      =(DATA_TYPE (*)[2*IMAGE_Y][2*IMAGE_X])output;
  DATA_TYPE (*check_out)[2*IMAGE_Y][2*IMAGE_X]=(DATA_TYPE (*)[2*IMAGE_Y][2*IMAGE_X])check_output;
  ...
  cl_kernel kernel = createKernel_Upsampling(program);
  
  
  /* Set Kernel arguments */
  //printf("-> Sets Kernel Args\n");
  setKernelArgs_Upsampling(kernel,
			   NB_WG0, NB_WG1, NB_WI,
			   outputBuffer,
			   IMAGE_X,
			   IMAGE_Y,
  ...
  #ifdef DATA_INT
  status = clEnqueueReadBuffer(commandQueue,outputBuffer,CL_TRUE,0,sizeof(cl_int)*4*IMAGE_X*IMAGE_Y,output,1,&event,NULL);
#else
  status = clEnqueueReadBuffer(commandQueue,outputBuffer,CL_TRUE,0,sizeof(cl_float)*4*IMAGE_X*IMAGE_Y,output,1,&event,NULL);
#endif
  ...
  printf("** OpenCL 'Upsampling' has completed. ** \n\n");
  ...
  copy_and_upsample_rows((float *)check_tmp,(float *)input,
			 IMAGE_X,IMAGE_Y);  
  copy_and_upsample_rows((float *)check_output,(float *)check_tmp,
			 IMAGE_Y,2*IMAGE_X);
  ...
  for(y=Y_MIN; y<2*Y_MAX ;y++) {
    for(x=X_MIN; x<2*X_MAX ;x++) {
#ifdef DATA_INT
  ...
  if (nok) {
    printf("ERROR on Upsampling filter verification !\n");
    // Input Matrix
    printf("\n");
    printf("Input sample \n");
    ...
    for(y=0;y<2*MIN(IMAGE_Y,PRINT_SIZE);y++) {
      for(x=0;x<2*MIN(IMAGE_X,PRINT_SIZE);x++) {
#ifdef DATA_INT
	printf(" %d\t", (*out)[y][x]);
#else
	printf(" %.2f", (*out)[y][x]);
    ...
    printf("\n");
     ...
     printf("Upsampling filter completed OK\n");		  
  }
