...
/* This is the test of the Convolution node */
...
float filterPattern[(DIM1_MAX)-(DIM1_MIN)+1][(DIM0_MAX)-(DIM0_MIN)+1] = { COEFS };
...
void computeConvolutionNxM(DATA_TYPE *output,
			   DATA_TYPE *input,
  ...
  if ((border==SKIP) || (border==UNDEF)) {
    X_MIN -= DIM0_MIN ;X_MAX -= DIM0_MAX;
    Y_MIN -= DIM1_MIN ;Y_MAX -= DIM1_MAX;
  }

  for(y=Y_MIN;y<Y_MAX;y++) {
    for(x=X_MIN;x<X_MAX;x++) {
      DATA_TYPE g=0;
      for (j=DIM1_MIN;j<=DIM1_MAX;j++) {
	for (i=DIM0_MIN;i<=DIM0_MAX;i++) {
	  g+=filterPattern[j-(DIM1_MIN)][i-(DIM0_MIN)]*ELEM(in,y+j,x+i);
	}
      }
      (*out)[y][x] = (g*multiplier)/divider;
    }
  }
}
...
int main(int argc, char * argv[]) {
...
#endif
  printf("\n");
  printf("  - Coefs : [%d:%d][%d:%d]\n",DIM1_MIN,DIM1_MAX,DIM0_MIN,DIM0_MAX);
  if (multiplier != 1.0) { printf("  - Multiplier : %f\n", multiplier); }
  if (divider != 1.0) { printf("  - Divider : %f\n", divider); }
  printf("  - Border mode: ");
  switch(BORDER) {
  case CONST_VALUE:
#ifdef DATA_INT
  ...
  cl_program program = createConvolutionProgramFromBinary( context, device);
#else
  /* create a CL program using the kernel source */
  cl_program program = createConvolutionProgramFromSource( context, device,
#ifdef DATA_INT
						     "-DDATA_INT"
#else
						     NULL
  ...
  cl_kernel kernel = createKernel_ConvolutionNxM(program);
  
  
  /* Set Kernel arguments */
  //printf("-> Sets Kernel Args\n");
  setKernelArgs_ConvolutionNxM(kernel,
			       NB_WG0, NB_WG1, NB_WI,
			       outputBuffer,
			       IMAGE_X,
			       IMAGE_Y,
  ...
  
  {
    ...
    printf("** OpenCL 'Convolution' has completed. ** \n\n");
...

  {
    ...
    computeConvolutionNxM(check_output,input,IMAGE_X,IMAGE_Y,BORDER);
  ...
  if (nok) {
    printf("ERROR on CONVOLUTION filter verification !\n");
    // Input Matrix
    printf("\n");
    printf("Input sample \n");
    ...
    printf("\n");
     ...
     printf("CONVOLUTION filter completed OK\n");		  
  }
