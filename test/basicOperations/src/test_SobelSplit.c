...
int main(int argc, char * argv[]) {
  ...
  cl_program program = createSobelSplitProgramFromBinary( context, device);
#else
  /* create a CL program using the kernel source */
  cl_program program = createSobelSplitProgramFromSource( context, device,
#ifdef DATA_INT
						     "-DDATA_INT"
#else
						     NULL
  ...
  cl_mem inputBuffer,outputBuffer,gxBuffer,gyBuffer;
#ifdef DATA_INT
  ...
  gxBuffer = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(cl_int)*IMAGE_X*IMAGE_Y,NULL,&status);
  oclCheckStatus(status,"gx buffer creation");
  gyBuffer = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(cl_int)*IMAGE_X*IMAGE_Y,NULL,&status);
  oclCheckStatus(status,"gy buffer creation");
  ...
  gxBuffer = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(cl_float)*IMAGE_X*IMAGE_Y,NULL,&status);
  oclCheckStatus(status,"gx buffer creation");
  gyBuffer = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(cl_float)*IMAGE_X*IMAGE_Y,NULL,&status);
  oclCheckStatus(status,"gy buffer creation");
  ...
  cl_kernel kernelgx = createKernel_Sobel3x3_Gx(program);
  cl_kernel kernelgy = createKernel_Sobel3x3_Gy(program);
  cl_kernel kernel   = createKernel_Sobel3x3_Merge(program);

  /* Set Kernel arguments */
  //printf("-> Sets Kernel Args\n");
  setKernelArgs_Sobel3x3_Gx(kernelgx,
			    NB_WG0, NB_WG1, NB_WI,
			    gxBuffer,
			    IMAGE_X,
			    IMAGE_Y,
  ...
  setKernelArgs_Sobel3x3_Gy(kernelgy,
			    NB_WG0, NB_WG1, NB_WI,
			    gyBuffer,
			    IMAGE_X,
			    IMAGE_Y,
  ...
  setKernelArgs_Sobel3x3_Merge(kernel,
			       NB_WG0, NB_WG1, NB_WI,
			       outputBuffer,
			       IMAGE_X,
			       IMAGE_Y,
...
oclCheckStatus(status,"clEnqueueReadBuffer failed.");
  ...
  clReleaseMemObject(gxBuffer);
  clReleaseMemObject(gyBuffer);
