...
#define FILTER_END 2
#ifdef DATA_INT
cl_int filter[5] = {7,31,52,31,7};
#else
//cl_float filter[5] = {0.34f, 2.5f, -3.12f, 1.76f, -0.56f};
cl_float filter[5] = {-0.56f, 1.76f, -3.12f,1.76f, -0.56f };
#endif
...
int main(int argc, char * argv[]) {
  ...
  cl_kernel kernel = createKernel_SeparableConvolution5x5(program);
  
  
  /* Set Kernel arguments */
  //printf("-> Sets Kernel Args\n");
  setKernelArgs_SeparableConvolution5x5(kernel,
					NB_WG0, NB_WG1, NB_WI,
					outputBuffer,
					IMAGE_X,
					IMAGE_Y,
  ...
  
  {
    ...
    printf("** OpenCL 'SeparableConvolution5x5' has completed. ** \n\n");
...

  {
    ...
    vl_imconvcol_v(check_tmp,IMAGE_Y,input,IMAGE_X,
		   IMAGE_Y,IMAGE_X,filter,FILTER_BEGIN,FILTER_END,1);
    vl_imconvcol_v(check_output,IMAGE_X,check_tmp,IMAGE_Y,
		   IMAGE_X,IMAGE_Y,filter,FILTER_BEGIN,FILTER_END,1);
  ...
  if (nok) {
    printf("ERROR on SeparableConvolution5x5 filter verification !\n");
    // Input Matrix
    printf("\n");
    printf("Input sample \n");
    ...
    printf("\n");
     ...
     printf("SeparableConvolution5x5 filter completed OK\n");		  
  }
