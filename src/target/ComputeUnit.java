...
public class ComputeUnit extends GenericDevice {
	...
	public boolean hasLocalCache() {
		return localCacheSize>=0;
	}
	public long getLocalCacheSize() {
		return localCacheSize;
	}
	public boolean hasLocalMemory() {
		return localMemorySize>=0;
	}
	...
	public String toString() {
		...
		// Cache/memory
		if (hasLocalCache()) {
			sb.append(", local cache = ").append(Long.toString(localCacheSize)).append(" bytes");
		}
		else {
			sb.append(", no local cache");
		}
		if (hasLocalMemory()) {
			sb.append(", local memory = ").append(Long.toString(localMemorySize)).append(" bytes");
		}
		else {
